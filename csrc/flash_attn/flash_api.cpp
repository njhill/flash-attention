...
mha_fwd(const at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
    ...
    set_params_fprop(params,
                     ...
                     /*start_seq_q_d=*/nullptr,
                     /*end_seq_q_d=*/nullptr,
                     /*start_seq_k_d=*/nullptr,
                     /*end_seq_k_d=*/nullptr,
...
mha_varlen_fwd(const at::Tensor &q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    ...
    TORCH_CHECK(k.dtype() == q_dtype, "query and key must have the same dtype");
    TORCH_CHECK(v.dtype() == q_dtype, "query and value must have the same dtype");
    TORCH_CHECK(start_seq_q.dtype() == torch::kInt32, "start_seq_q must have dtype int32");
    TORCH_CHECK(start_seq_k.dtype() == torch::kInt32, "start_seq_k must have dtype int32");
    TORCH_CHECK(end_seq_q.dtype() == torch::kInt32, "end_seq_q must have dtype int32");
    TORCH_CHECK(end_seq_k.dtype() == torch::kInt32, "end_seq_k must have dtype int32");

    TORCH_CHECK(q.is_cuda(), "Input tensor must be on CUDA device");
    TORCH_CHECK(k.is_cuda(), "Input tensor must be on CUDA device");
    TORCH_CHECK(v.is_cuda(), "Input tensor must be on CUDA device");
    TORCH_CHECK(start_seq_q.is_cuda(), "start_seq_q must be on CUDA device");
    TORCH_CHECK(start_seq_k.is_cuda(), "start_seq_k must be on CUDA device");
    TORCH_CHECK(end_seq_q.is_cuda(), "end_seq_q must be on CUDA device");
    TORCH_CHECK(end_seq_k.is_cuda(), "end_seq_k must be on CUDA device");

    TORCH_CHECK(q.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    TORCH_CHECK(k.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    TORCH_CHECK(v.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    TORCH_CHECK(start_seq_q.is_contiguous(), "start_seq_q must be contiguous");
    TORCH_CHECK(start_seq_k.is_contiguous(), "start_seq_k must be contiguous");
    TORCH_CHECK(end_seq_q.is_contiguous(), "end_seq_q must be contiguous");
    TORCH_CHECK(end_seq_k.is_contiguous(), "end_seq_k must be contiguous");
